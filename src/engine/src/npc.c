...
void drawNPC(TextureManager manager,SDL_Renderer* renderer,NPC npc,SDL_Rect camera){
    ...
    if(npc->direction == NPC_WEST){
        drawFrame(manager,"npc.png",npc->x - camera.x,npc->y - camera.y,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,2,npc->current_frame,renderer,SDL_FLIP_HORIZONTAL);

    } else if(npc->direction == NPC_EAST) {
        drawFrame(manager,"npc.png",npc->x - camera.x,npc->y - camera.y,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,2,npc->current_frame,renderer,SDL_FLIP_NONE);
    } else if(npc->direction == NPC_NORTH){
        drawFrame(manager,"npc.png",npc->x - camera.x,npc->y - camera.y,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,3,npc->current_frame,renderer,SDL_FLIP_NONE);
    } else if(npc->direction == NPC_SOUTH){
        drawFrame(manager,"npc.png",npc->x - camera.x,npc->y - camera.y,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,TILE_WIDTH,1,npc->current_frame,renderer,SDL_FLIP_NONE);
